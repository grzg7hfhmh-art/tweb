...
양성준 ❤️ 카바야마 사리 청첩장입니다.

https://grzg7hfhmh-art.github.io/tweb

2026년 6월 21일 오후 12시 30분, 진주 동방호텔 예식장 3층 그랜드홀에서 열리는 결혼식 청첩장입니다.
한국어와 일본어 지원이 포함되어 있습니다.
...
루트 디렉토리에 `.env` 파일을 생성하고 아래 환경변수를 설정합니다.
...
- `VITE_SERVER_URL`
  - 참석 의사 전달(상세 폼) API 서버 URL (선택 사항)
  - 서버 소스코드: https://github.com/juhonamnam/wedding-invitation-server
- `VITE_STATIC_ONLY`
  - 일부 동적 요소를 정적 모드로 표시하고 싶을 때 `true`로 설정 (선택 사항)
...
### 2026.01.22 - v0.1.2 (양성준/카바야마 사리 커스터마이징 버전)

- 한국어/일본어 다국어 지원 추가
- 결혼식 날짜를 2026년 6월 21일 오후 12시 30분으로 변경
- 예식장 위치를 진주 동방호텔 예식장 3층 그랜드홀로 변경
- 정보 컴포넌트 레이아웃 개선 (데스크톱에서 식사안내와 마음전하기를 나란히 배치)
